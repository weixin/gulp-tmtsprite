    ...
    async.waterfall([
      ...
      function (cb) {
        ...
        async.eachSeries(codelines, function (backgroundCodeLine, eachCb) {
          ...
          var absolutePath = path.join(css.base, relativePath)
          ...
          layerAddItem(layer, absolutePath, meta, function () {
            ...
            if (meta.hasRetina) {
              ...
              meta.className = classNameResult[0].replace(regexClassName, '$1');
        ...
        if (retinaLayerInfo.items.length > 0) {
          ...
          lwip.create(retinaLayerInfo.width, retinaLayerInfo.height, opt.color, function (err, image) {
            ...
            async.eachSeries(retinaLayerInfo.items, function (sprite, callback) {
              ...
              //添加 media query
              retinaCssContent += sprite.meta.className;
              retinaCssContent += '{background-image:url("../sprite/' + cssBaseName + '@2x.png");';
              retinaCssContent += 'background-position: -' + (sprite.x + sprite.meta.margin) + 'px -' + (sprite.y + sprite.meta.margin) + 'px;background-size:' + retinaLayerInfo.width / 2 + 'px;}';
...
//      console.log('file:', files[key].path)
//    }
//  })
