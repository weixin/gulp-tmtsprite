...
module.exports = function (options) {
  ...
  return through2.obj(function (css, enc, callback) {
    ...
    var color = Color(opt.background)
    opt.color = color.rgb().array()
    ...
    async.waterfall([
      ...
      function (cb) {
        ...
        async.eachSeries(codelines, function (backgroundCodeLine, eachCb) {
          ...
          // 统一为无引号
          var fixedQuoteReg = new RegExp('url\\(\\\\?["\']{1}(' + RegExp.escape(opt.slicePath) + '\\/[^\\\\"\']*)\\\\?["\']{1}\\)', 'i')
          ...
          //如果直接引用 @2x 图
          if (meta.backgroundCodeLine.indexOf('@2x') > 0) {
            meta.has2x = true
          ...
          //如果直接引用 @3x 图
          if (meta.backgroundCodeLine.indexOf('@3x') > 0) {
            meta.has3x = true
          ...
          //通过正则，匹配出 className
          var regexClassNameString = '(\\.?[^}]*?)\\s?{\\s?[^}]*?' + RegExp.escape(meta.backgroundCodeLine)
      ...
      function (cssContent, cb) {
        ...
        if (layerInfo.items.length > 0) {
          ...
          new jimp(layerInfo.width, layerInfo.height, opt.color[0], function(err, image){
      ...
      function (cssContent, cb) {
        ...
        if (retinaLayerInfo.items.length > 0) {
          ...
          new jimp(retinaLayerInfo.width, retinaLayerInfo.height, opt.color[0], function(err, image){
      ...
      function (cssContent, cb) {
        ...
        async.series([
          function (next) {
            if (image1x) {
              image1x.getBuffer(jimp.MIME_PNG, function (err, spriteBuffer) {
          ...
          function (next) {
            if (image2x) {
              image2x.getBuffer(jimp.MIME_PNG, function (err, retinaSpriteBuffer) {
          ...
          function (next) {
            if (image3x) {
              image3x.getBuffer(jimp.MIME_PNG, function (err, retinaSpriteBuffer) {
